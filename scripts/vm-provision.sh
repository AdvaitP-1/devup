#!/bin/bash
# DevUp VM provision script: install base toolchains (Node, Python, Go, C/C++).
...
if command -v node &>/dev/null && command -v python3 &>/dev/null && command -v go &>/dev/null && command -v gcc &>/dev/null; then
